...
// подключение элемента роутинга LINK
import { useCart } from '../hook/useCart ';

function Header({onClickCart ,items,favoritItems}){
    ...
    <ul className="d-flex">
        {/* вызывает компонент cart */}
        <li onClick={onClickCart} className="mr-30 cu-p">
            <div>
            <span className={items===0 ? 'hidden' : 'visible'}>
                {items}
            </span>
            <img className="cart" width={20} height={20} src="/img/cart.svg" alt="basket"/>
            </div>
        </li>
        <li className="mr-20 cu-p">
            <span className="text">{totalPrise} $</span>
        </li>
        <li className="mr-20 cu-p">
            {/* Ссылка на станицу фаворитов */}
            <Link className="mr-20 cu-p" to="/favorites">
                <span className={favoritItems===0 ? 'hidden' : 'visible'}>
                    {favoritItems}
                </span>
                <img className="cart" width={20} height={20} src="/img/favorite.svg" alt="basket"/>
            </Link>
        </li>
        <li>
            {/* Ссылка на стараницу заказов */}
            <Link className="mr-20 cu-p" to="/orders">
